...
Shared mode never deletes source videos and does not run the mutable channel-cleanup lifecycle. Once every shared command has exited successfully, run the encode command once without `--shared-work`. Its normal resume scan will find all outputs, build the final channel inventory, validate the archive, and offer the usual cleanup choice.
